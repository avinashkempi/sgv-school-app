...
export async function getFCMToken() {
  try {
    ...
    try {
      ...
      console.error(
        "❌ Cannot get native push token. Firebase notifications will not work."
      );
      console.error(
        "ℹ️ Use a Development Build or compiled APK/IPA to enable push notifications."
      );
      return null;
    }
    ...
    if (token.startsWith("ExponentPushToken")) {
      console.error(
        "❌ Received Expo Push Token. Firebase FCM does not support Expo tokens."
      );
      console.error(
        "ℹ️ Use a Development Build or compiled APK/IPA to enable push notifications."
      );
...
export async function unregisterFCMTokenFromBackend() {
  try {
    ...
    const response = await apiFetch(
      apiConfig.url(apiConfig.endpoints.fcm.unregister),
      {
        method: "POST",
        silent: true,
        headers: {

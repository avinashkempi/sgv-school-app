...
export const checkAppUpdate = async () => {
  ...
  if (Constants.appOwnership === "expo") {
    if (__DEV__) {
      console.log(
        "[InAppUpdates] Skipping check because we are running in Expo Go."
      );
    }
    return;
  }

  const { default: SpInAppUpdates, IAUUpdateKind } = await import(
  ...
  try {
    ...
    if (result.shouldUpdate) {
      if (__DEV__) {
        console.log(
          "[InAppUpdates] Update required! Target version:",
          result.storeVersion
        );
      }

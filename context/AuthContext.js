...
export const AuthProvider = ({ children }) => {
  ...
  const login = useCallback(async (newToken, newUser) => {
    try {
      ...
      const isAccountSwitch =
        previousUserId && previousUserId !== String(newUserId);
      ...
      await Promise.all([
        ...
        newUserId
          ? storage.setItem("@last_user_id", String(newUserId))
          : Promise.resolve(),
      ]);
      ...
      if (__DEV__) {
        console.log(
          `[AuthContext] Login complete${
            isAccountSwitch ? " (account switch detected)" : ""
          }`
        );
      }
    } catch (err) {
  ...
  const logout = useCallback(
    async (router, message = null, showToast = null) => {
      ...
      try {
        ...
        if (showToast && message) {
          ...
          showToast(
            message,
            isManualLogout ? "info" : "error",
            isManualLogout ? 1500 : 3500
          );

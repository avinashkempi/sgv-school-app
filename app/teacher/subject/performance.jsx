...
export default function SubjectPerformanceScreen() {
  ...
  const totalExamsWithData = performance.filter(
    (p) => p.overallAvgPercentage > 0
  ).length;
  const overallAvg =
    totalExamsWithData > 0
      ? (
          performance.reduce((sum, p) => sum + p.overallAvgPercentage, 0) /
          totalExamsWithData
        ).toFixed(1)
      : 0;

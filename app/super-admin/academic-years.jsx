...
export default function AcademicYearsScreen() {
    ...
    const doActivate = async () => {
      ...
      try {
        const response = await apiFetch(
          `${apiConfig.baseUrl}/academic-year/${yearId}/activate`,
          {
            method: "PUT",
          }
        );
  ...
  const handleStartTransition = (yearId) => {
    ...
    router.push(
      `/super-admin/transition-wizard?currentId=${currentYear._id}&nextId=${yearId}`
    );
  };
      ...
      >
        ...
        <View style={{ padding: 16 }}>
            ...
            >
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 12 }}>
                <StatItem
                  icon="people"
                  label="Students"
                  value={year.snapshot.totalStudents}
                  color={colors.primary}
                />
                <StatItem
                  icon="class"
                  label="Classes"
                  value={year.snapshot.totalClasses}
                  color={colors.secondary}
                />
                <StatItem
                  icon="school"
                  label="Exams"
                  value={year.snapshot.totalExams}
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            ...
            >
              <MaterialIcons
                name={isArchived ? "folder-open" : "visibility"}
                size={18}
                color={colors.onSurface}
              />
              <Text
              ...
              {!currentYear &&
                ...
                archivedYears.length === 0 && (
                  ...
                  >
                    ...
                    >
                      Create your first academic year to start managing your
                      school calendar
                    </Text>
                  </View>
                )}

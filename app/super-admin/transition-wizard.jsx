...
export default function TransitionWizardScreen() {
  ...
  const previewMutation = useApiMutation({
    mutationFn: createApiMutationFn(
      `${apiConfig.baseUrl}/academic-year/transition/preview`,
      "POST"
    ),
    onSuccess: (data) => {
      setImpactData(data);
  ...
  const executeMutation = useApiMutation({
    mutationFn: createApiMutationFn(
      `${apiConfig.baseUrl}/academic-year/transition/execute`,
      "POST"
    ),
    onSuccess: (data) => {
            ...
            {step < 3 && (
              <View
                style={{
                  ...
                  height: 2,
                  backgroundColor:
                    step < currentStep ? colors.success : colors.outlineVariant,
                  marginHorizontal: -20,
                  marginTop: -30,
                }}
              />
        ...
        <View style={{ marginTop: 24 }}>
          ...
          {isValidating ? (
            ...
            >
              ...
              {validationData.warnings.map((w, idx) => (
                <View
                  ...
                  style={{
                    ...
                    gap: 8,
                    marginBottom:
                      idx === validationData.warnings.length - 1 ? 0 : 10,
                  }}
                >
                  <MaterialIcons
                    name="error-outline"
              ...
              >
                It is recommended to resolve these issues before executing the
                transition.
              </Text>
            </View>
          ) : (
  ...
  const renderStep2Review = () => {
    ...
    const manualReviewStudents = promotionPlan.filter(
        ...
        >
          ...
          <View style={{ gap: 10 }}>
            <StatRow
              ...
              value={impact?.studentsAffected || 0}
              color={colors.primary}
            />
            <StatRow
              icon="trending-up"
              label="Will be Promoted"
              value={impact?.studentsPromoted || 0}
              color={colors.success}
            />
            <StatRow
              icon="school"
              label="Will Graduate"
              value={impact?.studentsGraduated || 0}
              color={colors.secondary}
            />
            <StatRow
              icon="flag"
              label="Need Manual Review"
              value={impact?.studentsNeedReview || 0}
              color={colors.error}
            />
          </View>
        </View>
        ...
        {manualReviewStudents.length > 0 && (
          <View style={{ marginBottom: 16 }}>
            ...
            {manualReviewStudents.length > 5 && (
              ...
              >
                ...and {manualReviewStudents.length - 5} more students need
                review
              </Text>
            )}
          </View>
    ...
    return (
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View style={{ gap: 8 }}>
            <CheckItem
              text={`Archive current year (${impactData.currentYear?.name})`}
            />
            <CheckItem
              text={`Promote ${
                impactData.impact?.studentsPromoted || 0
              } students`}
            />
            <CheckItem
              text={`Graduate ${
                impactData.impact?.studentsGraduated || 0
              } students`}
            />
            <CheckItem text="Reset all class rosters" />
            <CheckItem
              text={`Create ${
                impactData.impact?.studentsAffected || 0
              } historical records`}
            />
            <CheckItem
              text={`Activate new year (${impactData.nextYear?.name})`}
            />
          </View>
        </View>
          ...
          >
            <View
              style={{
                ...
                backgroundColor: createRollback
                  ? colors.primary
                  : "transparent",
                alignItems: "center",
                justifyContent: "center",
          ...
          >
            <View
              style={{
                ...
                borderRadius: 6,
                borderWidth: 2,
                borderColor: sendNotifications
                  ? colors.primary
                  : colors.outline,
                backgroundColor: sendNotifications
                  ? colors.primary
                  : "transparent",
                alignItems: "center",
                justifyContent: "center",
          ...
          >
            <View
              style={{
                ...
                backgroundColor: generateReport
                  ? colors.primary
                  : "transparent",
                alignItems: "center",
                justifyContent: "center",
        ...
        >
          ...
          <Pressable
            ...
            style={({ pressed }) => ({
              flex: 2,
              backgroundColor:
                !isConfirmed || executeMutation.isPending
                  ? colors.surfaceContainerHigh
                  : pressed
                  ? colors.error + "DD"
                  : colors.error,

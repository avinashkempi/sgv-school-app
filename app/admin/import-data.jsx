...
export default function ImportDataScreen() {
  ...
  const processLocalSync = async () => {
    ...
    try {
      const response = await apiFetch(
        `${apiConfig.baseUrl}/import/students/local`,
        {
          method: "POST",
          body: JSON.stringify({
            wipe: wipeData.toString(),
  ...
  const handleStaffImport = async () => {
    ...
    try {
      const response = await apiFetch(
        `${apiConfig.baseUrl}/import/staff/local`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
    ...
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 24 }}>
          ...
          >
            <TouchableOpacity
              ...
              style={{
                ...
                borderWidth: 1,
                borderColor:
                  selectedYearId === null ? colors.primary : colors.outline,
              }}
            >
              <Text
                style={{
            ...
            {years.map((yr) => (
              <TouchableOpacity
                ...
                style={{
                  ...
                  paddingVertical: 10,
                  backgroundColor:
                    selectedYearId === yr._id
                      ? colors.primary
                      : colors.surfaceVariant,
                  borderRadius: 12,
                  borderWidth: 1,
                  borderColor:
                    selectedYearId === yr._id ? colors.primary : colors.outline,
                }}
              >
                <Text
                  style={{
        ...
        <TouchableOpacity
          ...
          style={{
            backgroundColor: uploadingStudent
              ? colors.surfaceVariant
              : wipeData
              ? colors.error
              : colors.primary,
        ...
        <Card variant="elevated" style={{ marginBottom: 16 }}>
          ...
          <TouchableOpacity
            ...
            disabled={uploadingStudent || uploadingStaff}
            style={{
              backgroundColor: uploadingStaff
                ? colors.surfaceVariant
                : colors.secondary,
              padding: 14,
              borderRadius: 12,
        ...
        {result && (
          <View style={{ marginBottom: 30 }}>
            <Text style={[styles.titleMedium, { marginBottom: 12 }]}>
              Import Summary
            </Text>
            ...
            {result.classCounts &&
              Object.keys(result.classCounts).length > 0 && (
                <Card variant="outlined" style={{ marginBottom: 16 }}>
                  <Text style={[styles.titleSmall, { marginBottom: 12 }]}>
                    Class Distribution
                  </Text>
                  <View
                    style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}
                  >
                    {Object.entries(result.classCounts).map(
                      ([className, count]) => (
                        <View
                          key={className}
                          style={{
            ...
            {result.designationCounts &&
              Object.keys(result.designationCounts).length > 0 && (
                <Card variant="outlined" style={{ marginBottom: 16 }}>
                  <Text style={[styles.titleSmall, { marginBottom: 12 }]}>
                    Designation Summary
                  </Text>
                  <View
                    style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}
                  >
                    {Object.entries(result.designationCounts).map(
                      ([desig, count]) => (
                        <View
                          key={desig}
                          style={{
            ...
            {result.created > 0 && result.createdStaff && (
              <TouchableOpacity
                onPress={() =>
                  setResult({ ...result, showCreatedStaffModal: true })
                }
                style={{
            ...
            {result.updated > 0 && result.updatedStaff && (
              <TouchableOpacity
                onPress={() =>
                  setResult({ ...result, showUpdatedStaffModal: true })
                }
                style={{
                  backgroundColor: colors.secondary + "20",
                  padding: 12,
            ...
            <Modal
              ...
              transparent={true}
              onRequestClose={() =>
                setResult({ ...result, showUpdatedModal: false })
              }
            >
              <View
                style={{
                ...
                >
                  ...
                  >
                    <Text style={styles.titleMedium}>
                      Updated Students ({result.updated})
                    </Text>
                    <TouchableOpacity
                      onPress={() =>
                        setResult({ ...result, showUpdatedModal: false })
            ...
            <Modal
              ...
              transparent={true}
              onRequestClose={() =>
                setResult({ ...result, showCreatedStaffModal: false })
              }
            >
              <View
                style={{
            ...
            <Modal
              ...
              transparent={true}
              onRequestClose={() =>
                setResult({ ...result, showUpdatedStaffModal: false })
              }
            >
              <View
                style={{

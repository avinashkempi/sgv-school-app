...
export default function ManageSubjectsScreen() {
  ...
  } = useApiQuery(["adminSubjects"], `${apiConfig.baseUrl}/subjects`);
  ...
  const saveSubjectMutation = useApiMutation({
    ...
    onSuccess: () => {
      showToast(
        `Subject ${modalMode === "add" ? "created" : "updated"}`,
        "success"
      );
      setShowModal(false);
  ...
  const deleteSubjectMutation = useApiMutation({
    mutationFn: (id) =>
      createApiMutationFn(`${apiConfig.baseUrl}/subjects/${id}`, "DELETE")(),
    onSuccess: () => {
              ...
              {["Theory", "Practical", "Other"].map((type) => (
                <Pressable
                  ...
                  style={{
                    ...
                    paddingVertical: 8,
                    backgroundColor:
                      form.type === type ? colors.primary : colors.background,
                    borderRadius: 20,
                    borderWidth: 1,
                    borderColor:
                      form.type === type ? colors.primary : colors.border,
                  }}
                >
                  <Text
                    style={{

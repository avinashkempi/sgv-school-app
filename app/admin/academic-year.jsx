...
export default function AcademicYearScreen() {
    ...
    const { processedTeacherAttendance, teacherAttendanceSummary } = useMemo(() => {
        if (!reportData?.teacherAttendance || !Array.isArray(reportData.teacherAttendance)) {
            return { processedTeacherAttendance: [], teacherAttendanceSummary: reportData?.teacherAttendanceSummary || null };
        }

        const rawList = reportData.teacherAttendance;
        const isConsolidated = rawList.length > 0 && rawList[0].totalDays !== undefined;
        ...
        if (isConsolidated) {
            processed = rawList.map(item => {
                ...
                const pct = item.percentage !== undefined ? item.percentage : (total > 0 ? parseFloat(((present / total) * 100).toFixed(1)) : 0);
                return {
            ...
            rawList.forEach(item => {
                ...
                if (['present', 'late', 'excused', 'half-day'].includes(item.status)) {
            ...
            processed = Object.values(teacherMap).map(t => ({
                ...t,
                percentage: t.totalDays > 0 ? parseFloat(((t.presentDays / t.totalDays) * 100).toFixed(1)) : 0
            }));
        }

        // Calculate or retrieve summary
        const summary = reportData.teacherAttendanceSummary || (() => {
            const tracked = processed.filter(t => t.totalDays > 0);
            const avgPct = tracked.length > 0
                ? parseFloat((tracked.reduce((sum, t) => sum + t.percentage, 0) / tracked.length).toFixed(1))
                : 0;
            return {
                totalStaff: processed.length,
                trackedStaff: tracked.length,
                averagePercentage: avgPct,
                totalPresentDays: processed.reduce((sum, t) => sum + t.presentDays, 0),
                totalAbsentDays: processed.reduce((sum, t) => sum + t.absentDays, 0),
            };
        })();

        return { processedTeacherAttendance: processed, teacherAttendanceSummary: summary };
    ...
    const { classDistributionEntries, totalStudentsCount, totalClassesCount } = useMemo(() => {
        ...
        rawEntries.forEach(([_name, students]) => {
            const count = Array.isArray(students) ? students.length : (typeof students === 'number' ? students : (students?.count ?? 0));
        ...
        const studentsList = studentsResponse?.users || studentsResponse?.data || (Array.isArray(studentsResponse) ? studentsResponse : []);

        const isCurrentSelectedYearActive = years.find(y => y._id === selectedYearId)?.isActive;
        ...
        const matchingClasses = classesList.filter(cls => {
            ...
            const clsYearId = typeof cls.academicYear === 'object' ? cls.academicYear._id : cls.academicYear;
            return clsYearId === selectedYearId || (isCurrentSelectedYearActive && !clsYearId);
        });

        const targetClasses = matchingClasses.length > 0 ? matchingClasses : (isCurrentSelectedYearActive ? classesList : []);
        ...
        if (studentsList.length > 0) {
            const yearStudents = studentsList.filter(s => {
                const sYearId = typeof s.academicYear === 'object' ? s.academicYear?._id : s.academicYear;
                if (sYearId && sYearId === selectedYearId) return true;
                if (!sYearId && isCurrentSelectedYearActive) return true;
                if (s.currentClass) {
                    const sClassId = typeof s.currentClass === 'object' ? s.currentClass._id : s.currentClass;
                ...
                yearStudents.forEach(student => {
                    ...
                    if (student.currentClass) {
                        if (typeof student.currentClass === 'object') {
                            ...
                            const foundCls = targetClasses.find(c => c._id === student.currentClass);
        ...
        fallbackEntries.forEach(([_name, count]) => {
            totalCount += typeof count === 'number' ? count : (Array.isArray(count) ? count.length : 0);
        });
    ...
    }, [reportData?.classWiseStudents, reportData?.totalStudents, reportData?.totalClassesCount, initClassesData?.classes, studentsResponse, selectedYearId, years]);
                ...
                >
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
                        ...
                        <Text style={{
                            ...
                            {years.find(y => y._id === selectedYearId)?.name || "Select Academic Year"}
                        </Text>
                        {years.find(y => y._id === selectedYearId)?.isActive && (
                            <View style={{
                                backgroundColor: colors.primaryContainer || colors.primary + '18',
                                paddingHorizontal: 8,
                                paddingVertical: 2,
                                borderRadius: 6,
                            }}>
                ...
                <View>
                    ...
                    <View style={localStyles.card(colors)}>
                        ...
                        {classDistributionEntries.length > 0 && totalStudentsCount > 0 && (
                            <View style={{
                                flexDirection: "row",
                                backgroundColor: colors.surfaceContainer || colors.surfaceContainerHigh || '#f1f5f9',
                                borderRadius: 12,
                                padding: 12,
                                marginBottom: 16,
                                ...
                                <View style={{ width: 1, height: 24, backgroundColor: colors.outlineVariant + '40' }} />
                                <View style={{ alignItems: "center", flex: 1 }}>
                                    <Text style={{ fontSize: 18, fontFamily: "DMSans-Bold", color: colors.onSurface }}>
                                        {totalClassesCount}
                                    </Text>
                                ...
                                <View style={{ width: 1, height: 24, backgroundColor: colors.outlineVariant + '40' }} />
                                <View style={{ alignItems: "center", flex: 1 }}>
                                    <Text style={{ fontSize: 18, fontFamily: "DMSans-Bold", color: colors.secondary }}>
                                        {totalClassesCount > 0 ? (totalStudentsCount / totalClassesCount).toFixed(1) : 0}
                                    </Text>
                                    <Text style={{ fontSize: 12, fontFamily: "DMSans-Medium", color: colors.onSurfaceVariant, marginTop: 2 }}>
                                        Avg / Class
                                    </Text>
                        ...
                        {classDistributionEntries.length > 0 ? (
                            classDistributionEntries.map(([className, students], index) => {
                                ...
                                return (
                                    <View key={className} style={{
                                        ...
                                        alignItems: "center",
                                        paddingVertical: 10,
                                        borderBottomWidth: index < classDistributionEntries.length - 1 ? 1 : 0,
                                        borderBottomColor: colors.outlineVariant + '30'
                                    }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1, marginRight: 8 }}>
                                            <MaterialIcons name="school" size={18} color={colors.onSurfaceVariant} />
                                            <Text style={{ color: colors.onSurface, fontFamily: "DMSans-Medium", fontSize: 14, flex: 1 }}>
                                                {className}
                                            </Text>
                                        </View>
                                        <View style={{
                                            backgroundColor: count > 0 ? (colors.primaryContainer || colors.primary + '18') : (colors.surfaceContainerHigh || '#f1f5f9'),
                                            paddingHorizontal: 10,
                                            paddingVertical: 3,
                                            borderRadius: 8
                                        }}>
                                            <Text style={{
                                                color: count > 0 ? (colors.onPrimaryContainer || colors.primary) : colors.onSurfaceVariant,
                                                fontFamily: "DMSans-Bold",
                                                fontSize: 13
                                            }}>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, paddingVertical: 8 }}>
                                ...
                                <Text style={{ color: colors.onSurfaceVariant, fontFamily: "DMSans-Medium" }}>
                    ...
                    <View style={localStyles.card(colors)}>
                        ...
                        {/* Overall KPIs */}
                        {teacherAttendanceSummary && teacherAttendanceSummary.totalStaff > 0 && (
                            <View style={{
                                flexDirection: "row",
                                backgroundColor: colors.surfaceContainer || colors.surfaceContainerHigh || '#f1f5f9',
                                borderRadius: 12,
                                ...
                                <View style={{ alignItems: "center", flex: 1 }}>
                                    <Text style={{ fontSize: 18, fontFamily: "DMSans-Bold", color: colors.onSurface }}>
                                        {teacherAttendanceSummary.trackedStaff ?? 0}/{teacherAttendanceSummary.totalStaff ?? processedTeacherAttendance.length}
                                    </Text>
                                    <Text style={{ fontSize: 12, fontFamily: "DMSans-Medium", color: colors.onSurfaceVariant, marginTop: 2 }}>
                                        Staff Tracked
                                    </Text>
                        ...
                        {processedTeacherAttendance && processedTeacherAttendance.length > 0 ? (
                            processedTeacherAttendance.map((record, index) => {
                                ...
                                const badgeBg = record.totalDays === 0
                                ...
                                const badgeTextColor = record.totalDays === 0
                                ...
                                return (
                                    <View key={record.id || index} style={{
                                        paddingVertical: 10,
                                        borderBottomWidth: index < processedTeacherAttendance.length - 1 ? 1 : 0,
                                        ...
                                        <View style={{
                                            ...
                                            <View style={{ flex: 1, marginRight: 10 }}>
                                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                                    <Text style={{ color: colors.onSurface, fontFamily: "DMSans-Bold", fontSize: 14 }}>
                                                        {record.name}
                                                    </Text>
                                                    {displayRole ? (
                                                        <View style={{
                                                ...
                                                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 4, flexWrap: 'wrap' }}>
                                                    <Text style={{ color: colors.onSurfaceVariant, fontFamily: "DMSans-Regular", fontSize: 12 }}>
                                                        {record.totalDays > 0 ? `${record.presentDays} of ${record.totalDays} days` : 'No attendance recorded'}
                                                    </Text>
                                                    {record.absentDays > 0 && (
                                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                                                            <View style={{ width: 5, height: 5, borderRadius: 3, backgroundColor: colors.error || '#EF4444' }} />
                                                            <Text style={{ color: colors.error || '#EF4444', fontFamily: "DMSans-Medium", fontSize: 11 }}>
                                                    ...
                                                    {record.lateDays > 0 && (
                                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                                                            ...
                                                            <Text style={{ color: '#D97706', fontFamily: "DMSans-Medium", fontSize: 11 }}>
                                            ...
                                            <View style={{
                                                ...
                                                <Text style={{
                                                    ...
                                                    {record.totalDays > 0 ? `${record.percentage}%` : 'N/A'}
                                                </Text>
                                            </View>
                                        </View>
                                        ...
                                        {record.totalDays > 0 && (
                                            <View style={{
                                                ...
                                                <View style={{
                                                    height: '100%',
                                                    width: `${Math.min(Math.max(record.percentage, 0), 100)}%`,

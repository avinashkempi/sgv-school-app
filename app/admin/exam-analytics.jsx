...
export default function ExamAnalyticsScreen() {
    ...
    const { data: schoolData, isLoading, isFetching, refetch } = useApiQuery(
        ['schoolExamPerformance', selectedYearId],
        schoolPerfUrl,
    ...
    const studentsPerfUrl = useMemo(() => {
        ...
        if (searchStudentQuery.trim()) params.append('search', searchStudentQuery.trim());
        return `${apiConfig.baseUrl}/marks/analytics/school/students?${params.toString()}`;
    }, [selectedYearId, studentClassFilter, selectedExamType, searchStudentQuery]);

    const { data: studentsData, isLoading: loadingStudents, isFetching: fetchingStudents, refetch: refetchStudents } = useApiQuery(
        ['schoolStudentsRankings', selectedYearId, studentClassFilter, selectedExamType, searchStudentQuery],
        studentsPerfUrl,
    ...
    const onRefresh = async () => {
        setRefreshing(true);
        await Promise.all([refetch(), activeView === 'students' ? refetchStudents() : Promise.resolve()]);
        setRefreshing(false);
    };
    ...
    const examPerf = useMemo(() => schoolData?.examwisePerformance || [], [schoolData?.examwisePerformance]);
    const classPerf = useMemo(() => schoolData?.classwiseSummary || [], [schoolData?.classwiseSummary]);
    const subjectPerf = useMemo(() => schoolData?.subjectwiseSummary || [], [schoolData?.subjectwiseSummary]);
    const kpis = useMemo(() => schoolData?.kpis || {}, [schoolData?.kpis]);
    const initSummary = useMemo(() => schoolData?.initializationSummary || {}, [schoolData?.initializationSummary]);
    ...
    const activeKPIs = useMemo(() => {
        if (selectedExamType === 'ALL' || !currentExamTypePerf) {
            return {
                ...
                subLabel: `${kpis.totalMarksEntriesCount || 0} marks entered across ${kpis.totalExamsConfigured || 0} exams`
            };
        }
        return {
            ...
            subLabel: `Average ${currentExamTypePerf.avgMarksObtained || 0} / ${currentExamTypePerf.maxMarks || 100} M • ${currentExamTypePerf.marksEntered || 0} marks entered`
        };
    ...
    const filteredClasses = useMemo(() => {
        let list = classPerf.map(cls => {
            if (selectedExamType === 'ALL') {
                ...
                return {
                    ...
                    status: cls.examsCount >= 6 ? 'completed' : (cls.examsCount > 0 ? 'partial' : 'not_initialized')
                };
            } else {
                const b = (cls.examTypeBreakdown || []).find(x => x.examType === selectedExamType);
                const pct = b && b.avgPercentage !== null && b.avgPercentage !== undefined ? parseFloat(b.avgPercentage) : 0;
                return {
                    ...cls,
                    displayPercentage: pct,
                    ...
                    displayExamsCount: `${b?.examsCount || 0} ${selectedExamType} Assessment${b?.examsCount !== 1 ? 's' : ''}`,
        ...
        if (classSearchQuery.trim()) {
            ...
            list = list.filter(c => c.className.toLowerCase().includes(q) || (c.classTeacher && c.classTeacher.toLowerCase().includes(q)));
        }
    ...
    const filteredSubjects = useMemo(() => {
        let list = subjectPerf.map(s => {
            if (selectedExamType === 'ALL') {
                ...
                const b = (s.examTypeBreakdown || []).find(x => x.examType === selectedExamType);
                const pct = b && b.avgPercentage !== null && b.avgPercentage !== undefined ? parseFloat(b.avgPercentage) : 0;
                return {
                    ...s,
                    displayPercentage: pct,
    ...
    const validClasses = useMemo(() => filteredClasses.filter(c => c.displayPercentage > 0), [filteredClasses]);
    const topClass = validClasses.length > 0 ? validClasses[0] : null;
    const bottomClass = validClasses.length > 1 ? validClasses[validClasses.length - 1] : null;
    ...
    const renderOverview = () => (
        <Animated.View style={{ opacity: fadeAnim }}>
            {/* Hero Stats Card */}
            <View style={{ borderRadius: 24, overflow: 'hidden', marginTop: 16, elevation: 4 }}>
                <LinearGradient
                ...
                >
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                            ...
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                                {selectedExamType !== 'ALL' && (
                                    <View style={{
                                        ...
                                        <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: '#FFFFFF' }}>
                                            {selectedExamType}
                                        </Text>
                                    </View>
                                )}
                                <Text style={{
                                    ...
                                    {selectedExamType === 'ALL' ? 'School-wide Performance' : `${selectedExamType} Performance`}
                                </Text>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'baseline' }}>
                                <Text style={{ fontSize: 48, fontFamily: 'DMSans-Bold', color: colors.onPrimary, lineHeight: 54 }}>
                                    {activeKPIs.avgPercentage}
                                </Text>
                                <Text style={{ fontSize: 22, fontFamily: 'DMSans-Bold', color: colors.onPrimary, opacity: 0.8, marginLeft: 4 }}>
                                    %
                                </Text>
                            </View>
                        </View>
                        ...
                        <View style={{
                            ...
                            <Text style={{ fontSize: 10, fontFamily: 'DMSans-Bold', color: colors.onPrimary, opacity: 0.85, textTransform: 'uppercase' }}>
                                Total Marks Scored
                            </Text>
                            <Text style={{ fontSize: 15, fontFamily: 'DMSans-Bold', color: colors.onPrimary, marginTop: 2 }}>
                                {activeKPIs.totalMarksObtained?.toLocaleString() || 0} / {activeKPIs.totalMarksEvaluated?.toLocaleString() || 0}
                            </Text>
                        </View>
                    </View>
                    ...
                    <View style={{
                        ...
                        <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 10 }}>
                            <Text style={{ fontSize: 16, fontFamily: 'DMSans-Bold', color: colors.onPrimary }}>
                                {activeKPIs.examsConfigured || 0}
                            </Text>
                            <Text style={{ fontSize: 10, fontFamily: 'DMSans-Medium', color: colors.onPrimary, opacity: 0.8 }}>
                                {selectedExamType === 'ALL' ? 'Exams Setup' : `${selectedExamType} Setup`}
                            </Text>
                        </View>
                        <View style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: 10 }}>
                            <Text style={{ fontSize: 16, fontFamily: 'DMSans-Bold', color: colors.onPrimary }}>
            ...
            <View style={{ marginTop: 24 }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                    <View>
                        <Text style={{ fontSize: 17, fontFamily: 'DMSans-Bold', color: colors.onBackground }}>
                ...
                <View style={{ gap: 12 }}>
                    {filteredExamPerf.map((exam) => {
                        ...
                        return (
                            <Card key={exam.examType} variant="elevated" style={{ padding: 16 }}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                                        <View style={{
                                            ...
                                            <Text style={{ fontSize: 14, fontFamily: 'DMSans-Bold', color: examColor }}>
                                                {exam.examType}
                                            </Text>
                                        </View>
                                        ...
                                        <View style={{
                                            ...
                                            <Text style={{ fontSize: 12, fontFamily: 'DMSans-Bold', color: colors.onSurface }}>
                                                Max: {exam.maxMarks} Marks
                                            </Text>
                                        </View>

                                        <Text style={{ fontSize: 12, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant }}>
                                            {exam.examsCount} {exam.examsCount === 1 ? 'exam' : 'exams'}
                                        </Text>
                                    </View>

                                    {/* Percentage & Scored Marks */}
                                    <View style={{ alignItems: 'flex-end' }}>
                                        <Text style={{ fontSize: 18, fontFamily: 'DMSans-Bold', color: isEntered ? getGradeColor(pct) : colors.onSurfaceVariant }}>
                                            {isEntered ? `${pct}%` : '—'}
                                        </Text>
                                        <Text style={{ fontSize: 12, fontFamily: 'DMSans-Bold', color: colors.primary, marginTop: 2 }}>
                                            {isEntered ? `Avg: ${exam.avgMarksObtained} / ${exam.maxMarks}` : 'Pending marks'}
                                        </Text>
                                    </View>
                                </View>

                                {/* Progress Bar */}
                                <View style={{ height: 8, backgroundColor: colors.surfaceContainerHighest, borderRadius: 100, overflow: 'hidden', marginTop: 14 }}>
                                    <View style={{
                                        height: '100%',
                                        width: `${pct}%`,
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 10 }}>
                                    <Text style={{ fontSize: 11, fontFamily: 'DMSans-Regular', color: colors.onSurfaceVariant }}>
                                        Marks entered: <Text style={{ fontFamily: 'DMSans-Bold' }}>{exam.marksEntered} / {exam.expectedMarks}</Text> ({exam.completionRate}%)
                                    </Text>
                                    <Text style={{ fontSize: 11, fontFamily: 'DMSans-Regular', color: colors.onSurfaceVariant }}>
                                        Pool: <Text style={{ fontFamily: 'DMSans-Bold' }}>{exam.totalMarksObtained?.toLocaleString()} / {exam.totalMarksEvaluated?.toLocaleString()}</Text> M
                                    </Text>
                                </View>
                            </Card>
                        );
                ...
                <View style={{ flexDirection: 'row', gap: 12, marginTop: 20 }}>
                    <Card variant="outlined" style={{ flex: 1, padding: 14 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                            ...
                            <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: colors.success, textTransform: 'uppercase' }}>
                                Top Class
                            </Text>
                        </View>
                        ...
                        <Text style={{ fontSize: 18, fontFamily: 'DMSans-Bold', color: colors.success, marginTop: 4 }}>
                            {topClass.displayPercentage}%
                        </Text>
                        <Text style={{ fontSize: 11, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 2 }}>
                    ...
                    <Card variant="outlined" style={{ flex: 1, padding: 14 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                            ...
                            <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: '#FF9800', textTransform: 'uppercase' }}>
                                Needs Attention
                            </Text>
                        </View>
                        <Text style={{ fontSize: 15, fontFamily: 'DMSans-Bold', color: colors.onSurface, marginTop: 4 }}>
                        ...
                        <Text style={{ fontSize: 18, fontFamily: 'DMSans-Bold', color: '#FF9800', marginTop: 4 }}>
                            {bottomClass.displayPercentage}%
                        </Text>
                        <Text style={{ fontSize: 11, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 2 }}>
                            Avg {bottomClass.displayAvgMarks} / {bottomClass.displayMaxMarks} M
                        </Text>
                    </Card>
                </View>
            )}
        ...
        <View style={{ marginTop: 16 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        ...
                        {selectedExamType !== 'ALL' && (
                            <View style={{
                                ...
                                <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: getExamTypeColor(selectedExamType) }}>
                                    {selectedExamType}
                                </Text>
                            </View>
                        )}
            ...
            {filteredClasses.length === 0 ? (
                <Card variant="filled" style={{ padding: 36, alignItems: 'center' }}>
                    <MaterialIcons name="school" size={44} color={colors.onSurfaceVariant} style={{ opacity: 0.5 }} />
                    <Text style={{ fontSize: 14, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 10 }}>
                        No class data found
                    </Text>
                </Card>
            ) : (
                <View style={{ gap: 12 }}>
                    {filteredClasses.map((cls, index) => {
                        ...
                        const statusColors = {
                            ...
                            partial: { bg: '#FF980018', text: '#FF9800', label: `Partial (${cls.displayMarksEntered}/${cls.displayExpectedMarks})` },
                            pending: { bg: colors.primary + '18', text: colors.primary, label: 'Pending Marks' },
                            not_initialized: { bg: colors.surfaceContainerHighest, text: colors.onSurfaceVariant, label: 'Not Setup' }
                        };
                        const currentStatus = statusColors[cls.status] || statusColors.not_initialized;
                                ...
                                <Card variant="elevated" style={{ padding: 16 }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 }}>
                                            <View style={{
                                                ...
                                                height: 38,
                                                borderRadius: 19,
                                                backgroundColor: index === 0 ? colors.success + '18' : colors.primaryContainer,
                                                alignItems: 'center',
                                            ...
                                            <View style={{ flex: 1 }}>
                                                ...
                                                <Text style={{ fontSize: 12, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 2 }}>
                                                    {cls.studentCount} Students {cls.classTeacher ? `• ${cls.classTeacher}` : ''}
                                                </Text>
                                            </View>
                                        </View>
                                        ...
                                        <View style={{ alignItems: 'flex-end', gap: 4 }}>
                                            <View style={{
                                                ...
                                                <Text style={{ fontSize: 16, fontFamily: 'DMSans-Bold', color: getGradeColor(pct) }}>
                                                    {pct.toFixed(1)}%
                                                </Text>
                                            </View>
                                            <View style={{
                                                ...
                                                <Text style={{ fontSize: 10, fontFamily: 'DMSans-Bold', color: currentStatus.text }}>
                                                    {currentStatus.label}
                                                </Text>
                                            </View>
                                        </View>
                                    ...
                                    <View style={{
                                        ...
                                        <View>
                                            ...
                                            <Text style={{ fontSize: 14, fontFamily: 'DMSans-Bold', color: colors.primary, marginTop: 1 }}>
                                        ...
                                        <View style={{ alignItems: 'flex-end' }}>
                                            ...
                                            <Text style={{ fontSize: 13, fontFamily: 'DMSans-Bold', color: colors.onSurface, marginTop: 1 }}>
                                                {cls.displayMarksEntered} / {cls.displayExpectedMarks} Entries
                                            </Text>
                                        </View>
                                    </View>

                                    {/* Progress Bar */}
                                    <View style={{ height: 8, backgroundColor: colors.surfaceContainerHighest, borderRadius: 100, overflow: 'hidden', marginTop: 10 }}>
                                        <View style={{
                                            height: '100%',
    ...
    const renderSubjects = () => (
        <View style={{ marginTop: 16 }}>
            ...
            {filteredSubjects.length === 0 ? (
                <Card variant="filled" style={{ padding: 36, alignItems: 'center' }}>
                    <MaterialIcons name="book" size={44} color={colors.onSurfaceVariant} style={{ opacity: 0.5 }} />
                    <Text style={{ fontSize: 14, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 10 }}>
                ...
                <View style={{ gap: 12 }}>
                    {filteredSubjects.map((subj) => {
                        ...
                        return (
                            <Pressable key={subj.subjectId || subj.subjectName} onPress={() => setSelectedItem({ type: 'subject', data: subj })}>
                                <Card variant="elevated" style={{ padding: 16 }}>
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, flex: 1 }}>
                                            <View style={{
                                                ...
                                                <Text style={{ fontSize: 16, fontFamily: 'DMSans-Bold', color: colors.primary }}>
                                            ...
                                            <View style={{ flex: 1 }}>
                                                ...
                                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 2 }}>
                                                    <View style={{
                                                        ...
                                                        <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: colors.onSurfaceVariant }}>
                                        ...
                                        <View style={{ alignItems: 'flex-end' }}>
                                            ...
                                            <Text style={{ fontSize: 12, fontFamily: 'DMSans-Bold', color: colors.primary, marginTop: 3 }}>
                                    ...
                                    {subj.displayHighest !== null && subj.displayHighest !== undefined && (
                                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, paddingTop: 10, borderTopWidth: 1, borderTopColor: colors.outlineVariant + '30' }}>
                                            <Text style={{ fontSize: 11, fontFamily: 'DMSans-Medium', color: colors.success }}>
                                                Highest: <Text style={{ fontFamily: 'DMSans-Bold' }}>{subj.displayHighest} / {subj.displayMaxMarks} M</Text>
                                            </Text>
                                            {subj.displayLowest !== null && subj.displayLowest !== undefined && (
                                                <Text style={{ fontSize: 11, fontFamily: 'DMSans-Medium', color: colors.error }}>
                                                    Lowest: <Text style={{ fontFamily: 'DMSans-Bold' }}>{subj.displayLowest} / {subj.displayMaxMarks} M</Text>
                                                </Text>
                                            )}
                                        </View>
                                    )}
                                    ...
                                    {/* Progress bar */}
                                    <View style={{ height: 8, backgroundColor: colors.surfaceContainerHighest, borderRadius: 100, overflow: 'hidden', marginTop: 8 }}>
                                        <View style={{
                                            height: '100%',
                                            width: `${Math.min(pct, 100)}%`,
                                            backgroundColor: getGradeColor(pct),
    ...
    const renderStudents = () => (
        <View style={{ marginTop: 16 }}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
                <View style={{ flexDirection: 'row', gap: 8 }}>
                    <Pressable
                        ...
                        style={{
                            backgroundColor: !studentClassFilter ? colors.primary : colors.surfaceContainerHigh,
                            paddingHorizontal: 14,
                            paddingVertical: 6,
                            borderRadius: 20
                        }}
                    ...
                    {classPerf.map(c => (
                        <Pressable
                            ...
                            style={{
                                backgroundColor: studentClassFilter === c.classId ? colors.primary : colors.surfaceContainerHigh,
                                paddingHorizontal: 14,
                                paddingVertical: 6,
                                borderRadius: 20
                            }}
                        >
                            <Text style={{
                                fontSize: 12,
                                fontFamily: 'DMSans-Bold',
                                color: studentClassFilter === c.classId ? '#FFFFFF' : colors.onSurface
                            }}>
            ...
            {loadingStudents && !studentsData ? (
                ...
                <Card variant="filled" style={{ padding: 36, alignItems: 'center' }}>
                    <MaterialIcons name="person" size={44} color={colors.onSurfaceVariant} style={{ opacity: 0.5 }} />
                    <Text style={{ fontSize: 14, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 10 }}>
                ...
                <View style={{ gap: 10, opacity: fetchingStudents ? 0.85 : 1 }}>
                    {studentsList.map((student) => {
                        ...
                        const rankBadgeColor = student.rank === 1 ? '#FFD700' : (student.rank === 2 ? '#C0C0C0' : (student.rank === 3 ? '#CD7F32' : colors.primary));

                        return (
                            <Pressable key={student.studentId} onPress={() => setSelectedStudent(student)}>
                                <Card variant="elevated" style={{ padding: 14 }}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <View style={{
                                            ...
                                            height: 36,
                                            borderRadius: 18,
                                            backgroundColor: isTop3 ? rankBadgeColor + '25' : colors.surfaceContainerHigh,
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                            marginRight: 12
                                        }}>
                                        ...
                                        <View style={{ flex: 1 }}>
                                            ...
                                            <Text style={{ fontSize: 12, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 1 }}>
                                                {formatClassName(student.className)}
                                            </Text>
                                            {/* Exact Marks Scored Display */}
                                            <Text style={{ fontSize: 13, fontFamily: 'DMSans-Bold', color: colors.primary, marginTop: 3 }}>
                                                Scored: {student.totalObtained} / {student.totalMax} Marks
                                            </Text>
                                        </View>

                                        {/* Percentage & Grade */}
                                        <View style={{ alignItems: 'flex-end' }}>
                                            <Text style={{ fontSize: 18, fontFamily: 'DMSans-Bold', color: getGradeColor(student.percentage) }}>
                                                {student.percentage.toFixed(1)}%
                                            </Text>
                                            <View style={{
                                                backgroundColor: getGradeColor(student.percentage) + '18',
                                                paddingHorizontal: 8,
                                                paddingVertical: 2,
                                                borderRadius: 6,
                                                marginTop: 4
                                            }}>
                                                <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: getGradeColor(student.percentage) }}>
                                                    Grade {student.grade}
                                                </Text>
                                            </View>
                                        </View>
    ...
    const renderSetup = () => (
        <View style={{ marginTop: 16 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
                <Card variant="outlined" style={{ flex: 1, padding: 12, alignItems: 'center' }}>
                    <MaterialIcons name="check-circle" size={24} color={colors.success} />
                    <Text style={{ fontSize: 18, fontFamily: 'DMSans-Bold', color: colors.onSurface, marginTop: 4 }}>
                                ...
                                <View>
                                    ...
                                    <Text style={{ fontSize: 12, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant }}>
                                        {selectedExamType === 'ALL'
                                            ...
                                            : (breakdownMap[selectedExamType] ? `${selectedExamType} Initialized (${breakdownMap[selectedExamType].maxMarks} M) • ${cls.studentCount} Students` : `Not Initialized for ${selectedExamType}`)}
                                    </Text>
                                </View>
                                <Pressable
                                    onPress={() => router.push(`/shared/class-reports?classId=${cls.classId}`)}
                                    style={{
                                        backgroundColor: colors.primaryContainer,
                                        paddingHorizontal: 12,
                                        paddingVertical: 6,
                                    ...
                                    return (
                                        <View
                                            ...
                                            style={{
                                                ...
                                                borderColor: isSelected ? colors.primary : (isInit ? textColor + '40' : colors.outlineVariant + '30')
                                            }}
                                        >
                                            <Text style={{
                                                fontSize: 11,
                        ...
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                                <View style={{ flex: 1 }}>
                                    <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: colors.onSurfaceVariant, textTransform: 'uppercase', letterSpacing: 1 }}>
                                        {type === 'class' ? 'Class Performance Deep-Dive' : 'Subject Performance Deep-Dive'}
                                    </Text>
                                    <Text style={{ fontSize: 20, fontFamily: 'DMSans-Bold', color: colors.onSurface, marginTop: 4 }}>
                                        {title}
                                    </Text>
                                </View>
                                <View style={{ backgroundColor: getGradeColor(overallPct) + '18', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 14, alignItems: 'center' }}>
                                    <Text style={{ fontSize: 20, fontFamily: 'DMSans-Bold', color: getGradeColor(overallPct) }}>
                                        {overallPct}%
                                    </Text>
                                    <Text style={{ fontSize: 10, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant }}>
                                        Overall
                                    </Text>
                                </View>
                            </View>
                                    ...
                                    return (
                                        <View key={b.examType} style={{
                                            ...
                                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                                                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                                    <View style={{ backgroundColor: bColor + '20', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 6 }}>
                                                        <Text style={{ fontSize: 13, fontFamily: 'DMSans-Bold', color: bColor }}>
                                                            {b.examType}
                                                        </Text>
                                                    </View>
                                                    {/* Configured Max Marks */}
                                                    <View style={{ backgroundColor: colors.surfaceContainerHighest, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 }}>
                                            ...
                                            <View style={{ height: 6, backgroundColor: colors.surfaceContainerHighest, borderRadius: 10, overflow: 'hidden', marginTop: 4 }}>
                                                <View style={{
                                                    height: '100%',
                            ...
                            {type === 'class' && (
                                ...
                                >
                                    ...
                                    <Text style={{ fontSize: 15, fontFamily: 'DMSans-Bold', color: '#FFFFFF' }}>
                        ...
                        <View style={{
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                                <View style={{ flex: 1 }}>
                                    ...
                                    <Text style={{ fontSize: 14, fontFamily: 'DMSans-Bold', color: colors.primary, marginTop: 2 }}>
                                        Total Score: {s.totalObtained} / {s.totalMax} Marks ({s.percentage.toFixed(1)}%)
                                    </Text>
                                </View>
                                ...
                                <View style={{
                                    ...
                                    <Text style={{ fontSize: 20, fontFamily: 'DMSans-Bold', color: getGradeColor(s.percentage) }}>
                                        {s.percentage.toFixed(1)}%
                                    </Text>
                                    <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: getGradeColor(s.percentage) }}>
                                        Grade {s.grade}
                                    </Text>
                                </View>
                            </View>

                            <Text style={{ fontSize: 13, fontFamily: 'DMSans-Bold', color: colors.onSurfaceVariant, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 }}>
                            ...
                            <ScrollView showsVerticalScrollIndicator={false}>
                                {subjectScores.length === 0 ? (
                                    <View style={{ alignItems: 'center', paddingVertical: 24 }}>
                                        <MaterialIcons name="assignment" size={36} color={colors.onSurfaceVariant} style={{ opacity: 0.4 }} />
                                        <Text style={{ fontSize: 13, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, marginTop: 8 }}>
                                            ...
                                            >
                                                <View style={{ flex: 1 }}>
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 2 }}>
                                                        <View style={{
                                                            backgroundColor: getExamTypeColor(score.examType) + '20',
                                                            paddingHorizontal: 6,
                                                            paddingVertical: 2,
                                                            borderRadius: 4
                                                        }}>
                                                            <Text style={{ fontSize: 10, fontFamily: 'DMSans-Bold', color: getExamTypeColor(score.examType) }}>
                                                                {score.examType}
                                                            </Text>
                                                        </View>
                                                        <Text style={{ fontSize: 11, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant }}>
                                                ...
                                                <View style={{ alignItems: 'flex-end' }}>
                                                    <Text style={{ fontSize: 16, fontFamily: 'DMSans-Bold', color: getGradeColor(score.percentage) }}>
                                                        {score.marksObtained} / {score.totalMarks}
                                                    </Text>
                                                    <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: colors.onSurfaceVariant, marginTop: 1 }}>
            ...
            >
                    ...
                    {academicYears.length > 1 && (
                        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                            <View style={{ flexDirection: 'row', gap: 8, alignItems: 'center' }}>
                                <Text style={{ fontSize: 11, fontFamily: 'DMSans-Bold', color: colors.onSurfaceVariant, marginRight: 4 }}>
                                    YEAR:
                                </Text>
                                {academicYears.map(year => (
                                    <Pressable
                                        ...
                                        style={{
                                            backgroundColor: selectedYearId === year._id ? colors.primary : colors.surfaceContainerHigh,
                                            paddingHorizontal: 12,
                                            paddingVertical: 6,
                                            borderRadius: 16,
                                        }}
                                    >
                                        <Text style={{
                                            fontSize: 12,
                                            fontFamily: 'DMSans-Bold',
                                            color: selectedYearId === year._id ? '#FFFFFF' : colors.onSurface
                                        }}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                        <View style={{ flexDirection: 'row', gap: 6, alignItems: 'center' }}>
                            <Pressable
                                ...
                                style={{
                                    backgroundColor: selectedExamType === 'ALL' ? colors.primary : colors.surfaceContainerHigh,
                                    paddingHorizontal: 14,
                                    paddingVertical: 6,
                                    borderRadius: 20,
                                    borderWidth: selectedExamType === 'ALL' ? 1.5 : 0,
                                    borderColor: selectedExamType === 'ALL' ? '#FFFFFF40' : 'transparent'
                                }}
                            >
                                <Text style={{
                                    fontSize: 12,
                                    fontFamily: 'DMSans-Bold',
                                    color: selectedExamType === 'ALL' ? '#FFFFFF' : colors.onSurface
                                }}>
                                    All Exams
                                </Text>
                            </Pressable>
                            ...
                            {EXAM_TYPES.map(type => {
                                ...
                                return (
                                    <Pressable
                                        ...
                                        style={{
                                            backgroundColor: isSelected ? typeColor : colors.surfaceContainerHigh,
                                            paddingHorizontal: 12,
                                            paddingVertical: 6,
                                            borderRadius: 20,
                ...
                {isLoading && !schoolData ? (
                    <View style={{ paddingVertical: 48, alignItems: 'center', justifyContent: 'center' }}>
                        ...
                        <Text style={{ marginTop: 12, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant, fontSize: 14 }}>

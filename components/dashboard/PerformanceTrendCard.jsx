...
const getGradeInfo = (percentage) => {
    ...
    if (p >= 90) return { grade: 'A+', label: 'Outstanding', color: '#10B981', bg: 'rgba(16, 185, 129, 0.14)', border: 'rgba(16, 185, 129, 0.3)' };
    if (p >= 80) return { grade: 'A', label: 'Excellent', color: '#059669', bg: 'rgba(5, 150, 105, 0.14)', border: 'rgba(5, 150, 105, 0.3)' };
    if (p >= 70) return { grade: 'B+', label: 'Very Good', color: '#0284C7', bg: 'rgba(2, 132, 199, 0.14)', border: 'rgba(2, 132, 199, 0.3)' };
    if (p >= 60) return { grade: 'B', label: 'Good', color: '#6366F1', bg: 'rgba(99, 102, 241, 0.14)', border: 'rgba(99, 102, 241, 0.3)' };
    if (p >= 50) return { grade: 'C', label: 'Average', color: '#D97706', bg: 'rgba(217, 119, 6, 0.14)', border: 'rgba(217, 119, 6, 0.3)' };
    if (p >= 35) return { grade: 'D', label: 'Passing', color: '#EA580C', bg: 'rgba(234, 88, 12, 0.14)', border: 'rgba(234, 88, 12, 0.3)' };
    return { grade: 'F', label: 'Needs Focus', color: '#DC2626', bg: 'rgba(220, 38, 38, 0.14)', border: 'rgba(220, 38, 38, 0.3)' };
};
...
const PerformanceTrendCard = ({
    ...
    const normalizedData = (Array.isArray(data) ? data : [])
        ...
        .map((d, index) => {
            const rawPct = typeof d.percentage === 'number' ? d.percentage : (typeof d.value === 'number' ? d.value : 0);
            return {
    ...
    const trendDelta = previous ? Math.round((latest.percentage - previous.percentage) * 10) / 10 : null;
    const totalPercentage = normalizedData.reduce((acc, curr) => acc + curr.percentage, 0);
    const yearAverage = Math.round((totalPercentage / normalizedData.length) * 10) / 10;
    const bestExam = normalizedData.reduce((prev, curr) => (curr.percentage > prev.percentage ? curr : prev), normalizedData[0]);
                ...
                <Svg width="100%" height={svgHeight} viewBox={`0 0 ${effectiveWidth} ${svgHeight}`}>
                    <Defs>
                        ...
                        <LinearGradient id="performanceAreaGrad" x1="0" y1="0" x2="0" y2="1">
                            <Stop offset="0%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.45" />
                            <Stop offset="70%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.08" />
                            <Stop offset="100%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.00" />
                        </LinearGradient>

                        {/* Stroke Path Gradient */}
                        <LinearGradient id="performanceStrokeGrad" x1="0" y1="0" x2="1" y2="0">
                        ...
                        <LinearGradient id="activeNodeGlow" x1="0" y1="0" x2="0" y2="1">
                            <Stop offset="0%" stopColor={activeGrade.color} stopOpacity="0.6" />
                            <Stop offset="100%" stopColor={activeGrade.color} stopOpacity="0.1" />
                        </LinearGradient>
                    </Defs>
                        ...
                        return (
                            <G key={`point-${idx}`}>
                                ...
                                <Circle
                                    ...
                                    stroke={isSelected ? grade.color : (colors.primary || '#6750A4')}
                                ...
                                {(isSelected || points.length <= 4) && (
                                    <G>
                                        <Rect
                                            ...
                                            height={badgeHeight}
                                            rx="6"
                                            fill={isSelected ? grade.color : (isDark ? '#3E3947' : '#EDE7F6')}
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1, marginRight: 12 }}>
                        ...
                        >
                            ...
                            {activeExam.subjectCount > 0 ? ` (${activeExam.subjectCount} subjects)` : ''}
                        </Text>
                    </View>
                        ...
                        {activeIdx > 0 && (
                            <Text
                                style={{
                                    fontSize: 11,
                                    fontFamily: 'DMSans-Bold',
                                    color: activeExam.percentage >= normalizedData[activeIdx - 1].percentage ? '#10B981' : '#EF4444',
                                }}
                            >
                                {activeExam.percentage >= normalizedData[activeIdx - 1].percentage ? '▲' : '▼'}{' '}
                                {Math.abs(Math.round((activeExam.percentage - normalizedData[activeIdx - 1].percentage) * 10) / 10)}% vs {normalizedData[activeIdx - 1].examType}
                            </Text>
                        )}
                    </View>
                </View>

...
const getScoreGrade = (score, maxScore = 20) => {
    ...
    if (ratio >= 0.85 || s >= 16) {
        return { grade: 'A+', label: 'Outstanding', color: '#10B981', gradientFrom: '#10B981', gradientTo: '#059669', bg: 'rgba(16, 185, 129, 0.12)', border: 'rgba(16, 185, 129, 0.3)' };
    }
    if (ratio >= 0.70 || s >= 13) {
        return { grade: 'A', label: 'Excellent', color: '#0284C7', gradientFrom: '#38BDF8', gradientTo: '#0284C7', bg: 'rgba(2, 132, 199, 0.12)', border: 'rgba(2, 132, 199, 0.3)' };
    }
    if (ratio >= 0.50 || s >= 10) {
        return { grade: 'B+', label: 'Good', color: '#6366F1', gradientFrom: '#818CF8', gradientTo: '#4F46E5', bg: 'rgba(99, 102, 241, 0.12)', border: 'rgba(99, 102, 241, 0.3)' };
    }
    if (ratio >= 0.35 || s >= 7) {
        return { grade: 'C', label: 'Average', color: '#D97706', gradientFrom: '#FBBF24', gradientTo: '#D97706', bg: 'rgba(217, 119, 6, 0.12)', border: 'rgba(217, 119, 6, 0.3)' };
    }
    return { grade: 'D', label: 'Needs Focus', color: '#DC2626', gradientFrom: '#F87171', gradientTo: '#DC2626', bg: 'rgba(220, 38, 38, 0.12)', border: 'rgba(220, 38, 38, 0.3)' };
};
...
const TeacherPerformanceCard = ({
    ...
    rawLabels.forEach((lbl, idx) => {
        ...
        if (!aggregatedMap.has(cleanName)) {
            aggregatedMap.set(cleanName, { name: cleanName, originalLabel: lbl, total: marks, count: 1 });
        } else {
    ...
    const normalizedData = Array.from(aggregatedMap.values()).map((item, idx) => ({
        name: item.name,
    ...
    const topSubject = normalizedData.reduce((prev, curr) => (curr.marks > prev.marks ? curr : prev), normalizedData[0]);
    ...
    const activeIdx = selectedIndex !== null && selectedIndex >= 0 && selectedIndex < normalizedData.length
        ? selectedIndex
        : normalizedData.indexOf(topSubject) >= 0 ? normalizedData.indexOf(topSubject) : 0;
        ...
        >
            ...
            <View
                style={{
                    ...
                    marginBottom: 14,
                    borderWidth: 1,
                    borderColor: colors.outlineVariant ? colors.outlineVariant + '25' : 'rgba(0,0,0,0.06)',
                }}
            >
                <Pressable
                    ...
                    style={{
                        ...
                        paddingVertical: 6,
                        borderRadius: 8,
                        backgroundColor: viewMode === 'chart' ? (colors.primary || '#6750A4') : 'transparent',
                    }}
                >
                    <MaterialCommunityIcons
                        name="chart-bar"
                        size={16}
                        color={viewMode === 'chart' ? '#FFFFFF' : (colors.onSurfaceVariant || '#79747E')}
                        style={{ marginRight: 6 }}
                    />
                    <Text
                        style={{
                ...
                <Pressable
                    ...
                    style={{
                        ...
                        paddingVertical: 6,
                        borderRadius: 8,
                        backgroundColor: viewMode === 'list' ? (colors.primary || '#6750A4') : 'transparent',
                    }}
                >
                    <MaterialCommunityIcons
                        name="format-list-numbered"
                        size={16}
                        color={viewMode === 'list' ? '#FFFFFF' : (colors.onSurfaceVariant || '#79747E')}
                        style={{ marginRight: 6 }}
                    />
                    <Text
                        style={{
            ...
            {viewMode === 'chart' ? (
                ...
                >
                    <Svg width="100%" height={svgHeight} viewBox={`0 0 ${effectiveWidth} ${svgHeight}`}>
                            ...
                            {normalizedData.map((d, idx) => {
                                ...
                                return (
                                    <LinearGradient key={`barGrad-${idx}`} id={`barGrad-${idx}`} x1="0" y1="0" x2="0" y2="1">
                                        <Stop offset="0%" stopColor={g.gradientFrom} stopOpacity="1" />
                                        <Stop offset="100%" stopColor={g.gradientTo} stopOpacity="0.75" />
                                    </LinearGradient>
                                );
                            })}
                        </Defs>
                        ...
                        {yGridLevels.map((lvl) => {
                            const yPos = paddingTop + plotHeight - (lvl / chartMaxY) * plotHeight;
                            return (
                                <G key={`perf-grid-${lvl}`}>
                                    <Line
                                        x1={paddingLeft}
                        ...
                        <Line
                            ...
                            stroke={colors.outlineVariant ? colors.outlineVariant + '60' : 'rgba(0,0,0,0.15)'}
                            strokeWidth="1.5"
                        />
                        ...
                        {normalizedData.map((d, idx) => {
                            ...
                            const colCenter = paddingLeft + (idx * columnWidth) + (columnWidth / 2);
                            ...
                            return (
                                <G key={`bar-group-${idx}`}>
                                    ...
                                    <Rect
                                        ...
                                        height={badgeH}
                                        rx="6"
                                        fill={isSelected ? grade.color : (isDark ? '#383344' : '#EDE7F6')}
                        ...
                        return (
                            <Pressable
                                ...
                                style={({ pressed }) => ({
                                    ...
                                    padding: 12,
                                    borderWidth: 1,
                                    borderColor: isSelected ? grade.color : (colors.outlineVariant ? colors.outlineVariant + '25' : 'rgba(0,0,0,0.06)'),
                            ...
                            >
                                    ...
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, flex: 1 }}>
                                        ...
                                        <View
                                            style={{
                                                ...
                                                height: 26,
                                                borderRadius: 8,
                                                backgroundColor: rankIdx === 0 ? '#10B98120' : (rankIdx === 1 ? '#0284C720' : 'rgba(0,0,0,0.05)'),
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                            }}
                                        >
                                            <Text
                                                style={{
                                                    fontSize: 12,
                                                    fontFamily: 'DMSans-Bold',
                                                    color: rankIdx === 0 ? '#10B981' : (rankIdx === 1 ? '#0284C7' : (colors.onSurfaceVariant || '#79747E')),
                                                }}
                                            >
                                                #{rankIdx + 1}
                                            </Text>
                                ...
                                {/* Progress Track */}
                                <View style={{ height: 6, backgroundColor: colors.outlineVariant ? colors.outlineVariant + '30' : 'rgba(0,0,0,0.08)', borderRadius: 3, overflow: 'hidden' }}>
                                    <View
                                        style={{
            ...
            >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1, marginRight: 12 }}>
                        ...
                        >
                            {activeGrade.label} performance • {activeSubject.marks >= classAvg ? `+${(activeSubject.marks - classAvg).toFixed(1)} vs class avg` : `${(activeSubject.marks - classAvg).toFixed(1)} vs class avg`}
                        </Text>
                    </View>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                        ...
                        >
                            {activeSubject.marks} <Text style={{ fontSize: 13, fontFamily: 'DMSans-Medium', color: colors.onSurfaceVariant }}>/ {chartMaxY}</Text>
                        </Text>
                    </View>
                </View>
            </View>

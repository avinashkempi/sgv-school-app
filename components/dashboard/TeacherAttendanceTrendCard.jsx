...
const getAttendanceHealth = (pct) => {
    ...
    if (p >= 85) return { label: 'High Attendance', shortLabel: 'Excellent', color: '#10B981', bg: 'rgba(16, 185, 129, 0.12)', border: 'rgba(16, 185, 129, 0.3)' };
    if (p >= 75) return { label: 'Good Attendance', shortLabel: 'Good', color: '#0284C7', bg: 'rgba(2, 132, 199, 0.12)', border: 'rgba(2, 132, 199, 0.3)' };
    if (p >= 50) return { label: 'Moderate Attendance', shortLabel: 'Moderate', color: '#D97706', bg: 'rgba(217, 119, 6, 0.12)', border: 'rgba(217, 119, 6, 0.3)' };
    if (p > 0) return { label: 'Low Attendance', shortLabel: 'Low', color: '#DC2626', bg: 'rgba(220, 38, 38, 0.12)', border: 'rgba(220, 38, 38, 0.3)' };
    return { label: 'No Data / Holiday', shortLabel: 'No Data', color: '#9CA3AF', bg: 'rgba(156, 163, 175, 0.12)', border: 'rgba(156, 163, 175, 0.25)' };
};
...
const TeacherAttendanceTrendCard = ({
    ...
    const normalizedData = (Array.isArray(labels) ? labels : []).map((label, index) => {
        const rawVal = Array.isArray(data) && data[index] !== undefined ? data[index] : 0;
    ...
    const totalPercentage = (markedDays.length > 0 ? markedDays : normalizedData).reduce((acc, curr) => acc + curr.percentage, 0);
    const activeAverage = (markedDays.length > 0 ? markedDays : normalizedData).length > 0
        ? Math.round((totalPercentage / (markedDays.length > 0 ? markedDays : normalizedData).length) * 10) / 10
        : 0;

    // Peak attendance day
    const peakDay = normalizedData.reduce((prev, curr) => (curr.percentage > prev.percentage ? curr : prev), normalizedData[0]);
    ...
    const lastMarked = markedDays.length > 0 ? markedDays[markedDays.length - 1] : normalizedData[normalizedData.length - 1];
    const trendDelta = firstMarked && lastMarked && firstMarked !== lastMarked
        ? Math.round((lastMarked.percentage - firstMarked.percentage) * 10) / 10
        : null;

    // Active selected day (defaults to last day if none selected)
    const activeIdx = selectedIndex !== null && selectedIndex >= 0 && selectedIndex < normalizedData.length
        ? selectedIndex
    ...
    if (points.length === 1) {
        strokePath = `M ${points[0].x - 30} ${points[0].y} L ${points[0].x + 30} ${points[0].y}`;
        areaPath = `M ${points[0].x - 30} ${points[0].y} L ${points[0].x + 30} ${points[0].y} L ${points[0].x + 30} ${paddingTop + plotHeight} L ${points[0].x - 30} ${paddingTop + plotHeight} Z`;
        ...
        areaPath = `${strokePath} L ${points[1].x} ${paddingTop + plotHeight} L ${points[0].x} ${paddingTop + plotHeight} Z`;
    } else {
        ...
        areaPath = `${strokePath} L ${lastP.x} ${paddingTop + plotHeight} L ${firstP.x} ${paddingTop + plotHeight} Z`;
    }
    ...
    const handleNavigation = () => {
        if (onViewAttendance) {
            ...
            router.push({ pathname: '/teacher/class/attendance', params: { classId } });
        } else {
        ...
        >
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                ...
                >
                    ...
                    <MaterialIcons name="chevron-right" size={16} color={colors.primary || '#6750A4'} />
                </Pressable>
            </View>
            ...
            >
                ...
                <View
                    style={{
                        ...
                        borderRadius: 14,
                        borderWidth: 1,
                        borderColor: colors.outlineVariant ? colors.outlineVariant + '25' : 'rgba(0,0,0,0.05)',
                    }}
                >
                    <Text
                        style={{
                            fontSize: 11,
                            fontFamily: 'DMSans-Medium',
                            color: colors.onSurfaceVariant || (isDark ? '#CAC4D0' : '#79747E'),
                            marginBottom: 2,
                        }}
                    >
                        Avg Presence
                ...
                >
                    ...
                    {trendDelta !== null ? (
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <MaterialCommunityIcons
                                name={trendDelta >= 0 ? 'arrow-top-right' : 'arrow-bottom-right'}
                                size={18}
            ...
            <View
                style={{
                    ...
                    paddingHorizontal: 6,
                    borderWidth: 1,
                    borderColor: colors.outlineVariant ? colors.outlineVariant + '20' : 'rgba(0,0,0,0.04)',
                    overflow: 'hidden',
                }}
            >
                <Svg width="100%" height={svgHeight} viewBox={`0 0 ${effectiveWidth} ${svgHeight}`}>
                    <Defs>
                        ...
                        <LinearGradient id="attendanceAreaGrad" x1="0" y1="0" x2="0" y2="1">
                            <Stop offset="0%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.4" />
                            <Stop offset="65%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.08" />
                            <Stop offset="100%" stopColor={colors.primary || '#6750A4'} stopOpacity="0.00" />
                        </LinearGradient>

                        {/* Stroke Path Gradient */}
                        <LinearGradient id="attendanceStrokeGrad" x1="0" y1="0" x2="1" y2="0">
                        ...
                        <LinearGradient id="activeNodeGlowAtt" x1="0" y1="0" x2="0" y2="1">
                            <Stop offset="0%" stopColor={activeHealth.color} stopOpacity="0.6" />
                            <Stop offset="100%" stopColor={activeHealth.color} stopOpacity="0.1" />
                        </LinearGradient>
                    </Defs>
                        ...
                        return (
                            <G key={`att-grid-${lvl}`}>
                                ...
                                <SvgText
                                    x={paddingLeft - 6}
                                    y={yPos + 4}
                                    fill={colors.onSurfaceVariant || (isDark ? '#9E9E9E' : '#757575')}
                                    fontSize="10"
                                    fontFamily="DMSans-Medium"
                                    textAnchor="end"
                                    opacity="0.8"
                        ...
                        return (
                            <G key={`att-point-${idx}`}>
                                ...
                                <Circle
                                    ...
                                    stroke={isSelected ? health.color : (colors.primary || '#6750A4')}
                                ...
                                {(isSelected || (p.percentage === peakDay.percentage && peakDay.percentage > 0)) && (
                                    <G>
                                        <Rect
                                            x={badgeX}
                                            y={badgeY}
                                            ...
                                            height={badgeHeight}
                                            rx="6"
                                            fill={isSelected ? health.color : (isDark ? '#3E3947' : '#EDE7F6')}
                                        ...
                                        <SvgText
                                            x={p.x}
                                            y={badgeY + 14}
                                            fill={isSelected ? '#FFFFFF' : (isDark ? '#FFFFFF' : '#1D1B20')}
                                            fontSize="10.5"
                                            fontFamily="DMSans-Bold"
                                            fontWeight="bold"
                                            textAnchor="middle"
                                ...
                                <SvgText
                                    ...
                                    fill={isSelected ? (colors.primary || '#6750A4') : (colors.onSurfaceVariant || '#757575')}
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1, marginRight: 12 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                            ...
                            >
                                <Text style={{ fontSize: 10, fontFamily: 'DMSans-Bold', color: '#FFFFFF' }}>
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                        ...
                        {activeIdx > 0 && normalizedData[activeIdx - 1].percentage > 0 && activeDay.percentage > 0 && (
                            <Text
                                style={{
                                    fontSize: 11,
                                    fontFamily: 'DMSans-Bold',
                                    color: activeDay.percentage >= normalizedData[activeIdx - 1].percentage ? '#10B981' : '#EF4444',
                                }}
                            >
                                {activeDay.percentage >= normalizedData[activeIdx - 1].percentage ? '▲' : '▼'}{' '}
                                {Math.abs(Math.round((activeDay.percentage - normalizedData[activeIdx - 1].percentage) * 10) / 10)}% vs prev day
                            </Text>
                        )}
                    </View>
                </View>
                            ...
                            <Pressable
                                ...
                                style={({ pressed }) => ({
                                    ...
                                    borderColor: isSelected
                                        ? (colors.primary || '#6750A4')
                                        : (colors.outlineVariant ? colors.outlineVariant + '35' : 'rgba(0,0,0,0.08)'),
                                    opacity: pressed ? 0.75 : 1,
                                })}
                            >
                                <View
                                ...
                                <Text
                                    style={{
                                        fontSize: 12,
                                        fontFamily: 'DMSans-Bold',
                                        color: isSelected ? '#FFFFFF' : (colors.onSurface || (isDark ? '#FFFFFF' : '#1D1B20')),
                                    }}
                                >
                                    {item.label}
                                </Text>

...
export default function CSVImportModal({
  ...
  const importMutation = useApiMutation({
    mutationFn: createApiMutationFn(
      `${apiConfig.baseUrl}/marks/import/csv`,
      "POST"
    ),
    onSuccess: (result) => {
  ...
  const parseCSV = (text) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        ...
        else if (header.includes("mark"))
          row.marksObtained = values[index]?.trim();
  ...
  const handleImport = () => {
    if (!csvData || csvData.length === 0) {
      showToast(
        "No data to import. Please select a valid CSV file.",
        "warning"
      );
      return;
    }
                ...
                <>
                  ...
                  <Pressable
                    ...
                    style={({ pressed }) => ({
                      flex: 2,
                      backgroundColor:
                        csvData.length === 0 || importMutation.isPending
                          ? colors.surfaceContainerHigh
                          : pressed
                          ? colors.primary + "DD"
                          : colors.primary,

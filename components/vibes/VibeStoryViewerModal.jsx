...
import { getFeedImageUrl, getStoryThumbnailUrl, getBlurPlaceholderUrl } from '../../utils/cloudinaryUpload';
 ...
 */
const StoryProgressBar = memo(({ index, currentIndex, isPaused, isMediaLoaded, onSegmentComplete }) => {
  ...
  }, [currentIndex, index, isPaused, isMediaLoaded, progress, onSegmentComplete]);
...
const VibeStoryViewerModal = ({
  ...
  useEffect(() => {
    if (visible && currentVibe?._id && !viewedIdsRef.current.has(currentVibe._id)) {
      viewedIdsRef.current.add(currentVibe._id);
      
      if (isAuthenticated) {
        createApiMutationFn(
          `${apiConfig.baseUrl}${apiConfig.endpoints.vibes.recordView(currentVibe._id)}`,
          'POST'
        )({}).catch(() => {});
      }
    }
  ...
  const handleTouchZonePress = useCallback((isRightSide) => {
    const now = Date.now();
  ...
  const panGesture = Gesture.Pan()
    .onUpdate((e) => {
      ...
      if (e.translationY > 0 && Math.abs(e.translationY) > Math.abs(e.translationX) * 0.8) {
  ...
  const optimizedImage = mediaUrl ? getFeedImageUrl(mediaUrl, { isSlow }) : null;
  const authorAvatar = currentVibe.author?.profilePhoto ? getStoryThumbnailUrl(currentVibe.author.profilePhoto) : null;
  const authorAvatarBlur = authorAvatar ? getBlurPlaceholderUrl(authorAvatar) : null;
        ...
        <GestureDetector gesture={panGesture}>
          <Animated.View style={[styles.modalRoot, modalAnimatedStyle]}>
            ...
            <View style={styles.bottomControls}>
              {currentVibe.caption ? (
                <BlurView intensity={Platform.OS === 'ios' ? 45 : 80} tint="dark" style={styles.captionGlassBox}>
...
const styles = StyleSheet.create({
  ...
  captionGlassBox: {
    ...
    backgroundColor: Platform.OS === 'android' ? 'rgba(0,0,0,0.65)' : 'transparent',
  },
  captionText: {
    color: '#fff',
    fontSize: 13,

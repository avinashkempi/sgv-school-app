...
const PostFeed = ({ category, onEditPost }) => {
  ...
  const {
    ...
    queryKey,
    (page) =>
      `${apiConfig.baseUrl}${apiConfig.endpoints.posts.list}?category=${category}&page=${page}&limit=${POSTS_PER_PAGE}`,
    {
        ...
        >
          {isAdmin
            ? `Tap the + button to add your first ${
                category === "achievement" ? "achievement" : "post"
              }`
            : `${
                category === "achievement" ? "Achievements" : "Posts"
